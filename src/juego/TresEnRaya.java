 ...
 *
 * @version 28-05-2018
 * @author Carlos Aguirre Vozmediano Gráficos: Java Swing. Juego: Tres en raya y
 * tres en raya tres dimensiones.
 */
public class TresEnRaya {
     ...
     *
     * @param pJuegoNormal Especifica si se trata de un tres en raya normal
     * (true) o tres en raya en tres dimensiones (false).
     */
    public TresEnRaya(boolean pJuegoNormal) {
        this.juegoNormal = pJuegoNormal;
        if (this.juegoNormal) {
            this.tablero = new byte[LONGITUD][LONGITUD][1];
        } else {
            this.tablero = new byte[LONGITUD][LONGITUD][LONGITUD];
        }
        this.jugador = true;
    ...
    /**
     * Controla si se puede poner una ficha en una determinada coordenada, la
     * coloca si puede y comprueba si hay un ganador.
     *
    ...
    /**
     * Este método añade una ficha al tablero si está vacia, devuelve True si se
     * ha podido poner la ficha y False si ya estaba ocupado esa posición.
     *
    ...
    private boolean setFicha(byte x, byte y) {
        if (tablero[x][y][0] == 0) {
            tablero[x][y][0] = this.jugador ? (byte) -1 : (byte) 1;
            return true;
        } else {
            return false;
        }
    ...
    private boolean setFicha(byte x, byte y, byte z) {
        if (tablero[x][y][z] == 0) {
            tablero[x][y][z] = this.jugador ? (byte) -1 : (byte) 1;
            return true;
        } else {
            return false;
        }
    ...
    /**
     * Define el campo 'ganador' que es quien ha ganado la partida. '0' -
     * Empate. '-1' - Gana J1 '2' - Gana J2
     *
     * @param valor Tipo (byte) admite solo dos valores -3 ó 3.
     */
    private void setGanador(byte valor) {
        this.ganador = (valor == -3 ? (byte) -1 : (byte) 1);
    }

    /**
     * Comprueba filas y columnas al mismo tiempo para determinar si hay un
     * ganador. Si hay ganador se define el atributo 'finPartida' a True, se
     * define el ganador y se almacena la coordenada que hizo posible ganar.
     */
            ...
            if (this.juegoNormal) {
                for (byte x = 0; x < this.LONGITUD; x++) {
                    ...
                    for (byte y = 0; y < this.LONGITUD; y++) {
                        if (this.tablero[x][y][0] != 0) {
                            filas += this.tablero[x][y][0];
                        }
                        if (this.tablero[y][x][0] != 0) {
                            columnas += this.tablero[y][x][0];
                        }
                    }
                    if (filas == -3 || filas == +3) {
                ...
                if (!detectadoGanador) {
                    ...
                    if (b == -3 || b == +3) {
                        this.auxiliarComprobador(b);
                        this.corGanadora = 5;
                    }
                }

                if (!detectadoGanador && this.turno >= 9) {
                    ...
                    for (byte x = 0; x < this.LONGITUD; x++) {
                        ...
                        for (byte y = 0; y < this.LONGITUD; y++) {
                            if (this.tablero[x][y][z] != 0) {
                                filas += this.tablero[x][y][z];
                            }
                            if (this.tablero[y][x][z] != 0) {
                                columnas += this.tablero[y][x][z];
                            }
                        }
                        if (filas == -3 || filas == +3) {
                    ...
                    for (byte x = 0; x < this.LONGITUD; x++) {
                        for (byte y = 0; y < this.LONGITUD; y++) {
                            ...
                            for (byte z = 0; z < this.LONGITUD; z++) {
                                if (this.tablero[x][y][z] != 0) {
                                    suma += this.tablero[x][y][z];
                                }
                                if (suma == 3 || suma == -3) {
                                    this.auxiliarComprobador(suma, (byte) 3);
                                    switch (y) {
                                        case 0:
                                            this.corGanadora = (byte) (6 + x + y);
                                            break;
                                        case 1:
                                            this.corGanadora = (byte) (6 + (x + 1) + (y * 2));
                                            break;
                                        case 2:
                                            this.corGanadora = (byte) (6 + (x + 2) + (y * 2));
                                            break;
                                        default:
                                            break;
                                    }
                ...
                if (!detectadoGanador) {
                    for (byte x = 0; x < this.LONGITUD; x++) {
                        for (byte y = 0; y < this.LONGITUD; y++) {
                            ...
                            if (x == 1 && y == 1) {
                                continue;
                            } // Para que se salte la columna del centro que ya la ha comprobado anteriormente.
                            if (this.tablero[x][y][0] != 0) {
                                diagonalFija += this.tablero[x][y][0];
                            }
                            if (this.tablero[1][1][1] != 0) {
                                diagonalFija += this.tablero[1][1][1];
                            } // Centro, es fijo.
                            byte xContrario = x;
                            byte yContrario = y;
                            if (x == 0) {
                            ...
                            if (this.tablero[xContrario][yContrario][2] != 0) {
                                diagonalFija += this.tablero[xContrario][yContrario][2];
                            }
                ...
                if (!detectadoGanador) {
                    ...
                    if (this.tablero[0][0][0] != 0) {
                        diagonalHoriVer += this.tablero[0][0][0];
                    }
                    if (this.tablero[1][0][1] != 0) {
                        diagonalHoriVer += this.tablero[1][0][1];
                    }
                    if (this.tablero[2][0][2] != 0) {
                        diagonalHoriVer += this.tablero[2][0][2];
                    }
                    ...
                    if (!detectadoGanador) {
                        ...
                        if (this.tablero[0][1][1] != 0) {
                            diagonalHoriVer += this.tablero[0][1][1];
                        }
                        if (this.tablero[0][2][2] != 0) {
                            diagonalHoriVer += this.tablero[0][2][2];
                        }
                        if (diagonalHoriVer == 3 || diagonalHoriVer == -3) {
                            this.auxiliarComprobador(diagonalHoriVer, (byte) 3);
                            this.corGanadora = 24;
                    ...
                    if (!detectadoGanador) {
                        diagonalHoriVer = 0;
                        if (this.tablero[2][0][0] != 0) {
                            diagonalHoriVer += this.tablero[2][0][0];
                        }
                        if (this.tablero[2][1][1] != 0) {
                            diagonalHoriVer += this.tablero[2][1][1];
                        }
                        if (this.tablero[2][2][2] != 0) {
                            diagonalHoriVer += this.tablero[2][2][2];
                        }
                        if (diagonalHoriVer == 3 || diagonalHoriVer == -3) {
                            this.auxiliarComprobador(diagonalHoriVer, (byte) 3);
                            this.corGanadora = 25;
                    ...
                    if (!detectadoGanador) {
                        ...
                        if (this.tablero[0][0][2] != 0) {
                            diagonalHoriVer += this.tablero[0][0][2];
                        }
                        if (diagonalHoriVer == 3 || diagonalHoriVer == -3) {
                            this.auxiliarComprobador(diagonalHoriVer, (byte) 3);
                            this.corGanadora = 26;
                    ...
                    if (!detectadoGanador) {
                        diagonalHoriVer = 0;
                        if (this.tablero[2][2][0] != 0) {
                            diagonalHoriVer += this.tablero[2][2][0];
                        }
                        if (this.tablero[1][2][1] != 0) {
                            diagonalHoriVer += this.tablero[1][2][1];
                        }
                        if (this.tablero[0][2][2] != 0) {
                            diagonalHoriVer += this.tablero[0][2][2];
                        }
                    ...
                    if (!detectadoGanador) {
                        diagonalHoriVer = 0;
                        if (this.tablero[0][2][0] != 0) {
                            diagonalHoriVer += this.tablero[0][2][0];
                        }
                        if (this.tablero[0][1][1] != 0) {
                            diagonalHoriVer += this.tablero[0][1][1];
                        }
                ...
                if (!detectadoGanador && this.turno >= 27) {
                    this.finPartida = true;
                }
            }
        }
    ...
    /**
     * Ayuda a quitar la redundancia de código del método comprobarTablero() del
     * tres en raya normal.
     *
     * @param ganador Jugador que ha ganado la partida (-1 (J1), 0 (Empate), 1
     * (J2)).
     */
    ...
    /**
     * Ayuda a quitar la redundancia de código del método comprobarTablero() del
     * tres en raya en tres dimensiones.
     *
     * @param ganador Jugador que ha ganado la partida (-1 (J1), 0 (Empate), 1
     * (J2)).
     * @param tabGanador Tablero que tiene la jugada ganadora: -1 (nadie gana,
     * empate), 0 (1º Tablero), 1 (2º Tablero), 2 (3º Tablero), 3 (Combinación
     * de los tres).
     */
    ...
    /**
     * Establece todos los atributos y las variables del tablero (Array
     * tridimensional) a 0.
     */
    ...
    /**
     * Devuelve el tablero que ha ganado la partida. Tablero ganador puede ser:
     * -1 (nadie gana, empate), 0 (1º Tablero), 1 (2º Tablero), 2 (3º Tablero),
     * 3 (Combinación de los tres)
     *
     * @return byte
     */
    public byte getTableroGanador() {
        return this.tableroGanador;
    }
     ...
     *
     * @return (-3 - 5) Coordenada ganadora: x(-1,-2,-3),y(1,2,3),
     * abajo-izquierda a arriba-derecha(4), abajo-derecha a arriba izquierda(5)
     */
    public byte getCoordenadaGanadora() {
        return this.corGanadora;
    }
    ...
    public byte getValorCasilla(byte x, byte y, byte z) {
        if (this.verificarCoordenadas(x, y, z)) {
    ...
    public boolean verificarCoordenadas(byte x, byte y) {
        if (x >= 0 || x < this.LONGITUD && y >= 0 || y < this.LONGITUD) {
            return true;
        } else {
            return false;
        }
    ...
    public boolean verificarCoordenadas(byte x, byte y, byte z) {
        if (x >= 0 || x < this.LONGITUD && y >= 0 || y < this.LONGITUD && z >= 0 || z < this.LONGITUD) {
            return true;
        } else {
            return false;
        }
    ...
    /**
     * Muestra el tablero por consola de texto, este método es solo para
     * comprobaciones que realiza el desarrollador.
     */
